    ...
    <div className="w-full h-full bg-[var(--color-brand-navy)] border border-[var(--color-brand-gold)]/30 flex items-center justify-center">
      <div className="flex flex-col items-center" style={{ width: '160px', gap: '0' }}>
        ...
        >
          ...
          <div className="grid flex-shrink-0" style={{ gridTemplateColumns: 'repeat(3, 1fr)', gap: '8px' }}>
            {Array.from({ length: 12 }).map((_, i) => (
              <div key={i} className="rounded-full bg-[var(--color-brand-gold)]/20 border border-[var(--color-brand-gold)]/30" style={{ width: '14px', height: '14px' }} />
            ))}
          </div>
        </div>
        <div className="bg-[var(--color-brand-gold)]/20 rounded-b-sm" style={{ width: '83.33%', height: '12px' }} />
        <div className="bg-[var(--color-brand-gold)]/10" style={{ width: '50%', height: '8px' }} />
      </div>
    </div>
  );
}
  ...
  return (
    <div className="w-full bg-[var(--color-brand-black)]">
      <PageHero
        ...
        imageAlt="Financial services and technology"
      />
        ...
        <div className="max-w-[1440px] mx-auto px-6 md:px-12 flex flex-col md:flex-row items-center gap-16">
          <div className="md:w-1/2">
            <FadeIn>
              ...
              >
                {T.common.learnGroup} <ArrowRight size={16} className="group-hover:translate-x-1 transition-transform" />
              </Link>
            </FadeIn>
          </div>
